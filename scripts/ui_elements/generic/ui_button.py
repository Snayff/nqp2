...
class UIButton:
    ...
    def draw(self, surf, offset=(0, 0)):
        ...
        if type(self.content) != str:
            ...
            self._game.visuals._fonts["default"].draw(
                self.content,
                surf,
                (
                    self.pos[0]
                    + offset[0]
                    + (self.size[0] - self._game.visuals._fonts["default"].width(self.content)) // 2,
                    self.pos[1] + offset[1] + (self.size[1] - self._game.visuals._fonts["default"].line_height) // 2,
                ),
            )
