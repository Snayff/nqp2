...
class CombatUI(UI):
    def __init__(self, game: Game, parent_scene: CombatScene):
        ...
        self.place_target = [
            self._game.window.base_resolution[0] // 8,
            self._game.window.base_resolution[1] // 2,
        ]
        self.selected_col = 0
    ...
    def update(self, delta_time: float):
        ...
        if (self._game.combat.general_state == "actions") and (self._game.combat.combat_ending_timer == -1):
            target_pos = self._game.combat.get_team_center("player")
            if not target_pos:
                target_pos = [0, 0]
            else:
            ...
            self._game.combat.camera.pos[0] += (
                (target_pos[0] - self._game.combat.camera.pos[0]) / 10 * (self._game.window.delta_time * 60)
            )
            self._game.combat.camera.pos[1] += (
                (target_pos[1] - self._game.combat.camera.pos[1]) / 10 * (self._game.window.delta_time * 60)
            )
    ...
    def process_input(self, delta_time: float):
        ...
        if self._game.combat.state in [CombatState.UNIT_CHOOSE_CARD, CombatState.ACTION_CHOOSE_CARD]:
            ...
            if self._game.input.states["select"]:
                ...
                if self._game.combat.state == CombatState.UNIT_CHOOSE_CARD:
                    # can only use a card if there are cards in your hand and point limit not hit
                    if self._game.combat.units_are_available[self.selected_col]:
                        unit = self._game.memory.player_troupe._units[
                            self._game.combat.placeable_units[self.selected_col]
                        ]
                        leadership = self._game.memory.leadership
                        leadership_remaining = leadership - self._game.combat.leadership_points_spent
                        if self.button_count and leadership_remaining >= unit.tier:
                            self._game.combat.state = CombatState.UNIT_SELECT_TARGET
                else:
                    if not self._game.combat.skill_cooldowns[self.selected_col]:
                        # determine action target mode
                        target_type = action = self._game.combat.actions[
                            self._game.memory.player_actions[self.selected_col]
                        ](self._game).target_type
                        if target_type == "free":
                            self._game.combat.state = CombatState.ACTION_SELECT_TARGET_FREE
        ...
        elif self._game.combat.state in [CombatState.UNIT_SELECT_TARGET, CombatState.ACTION_SELECT_TARGET_FREE]:
            directions = {
                "right": (1, 0),
                "left": (-1, 0),
                "up": (0, -1),
            ...
            for direction in directions:
                if self._game.input.states["hold_" + direction]:
                    offset(move_amount, directions[direction], self._game.window.delta_time * 175)
            ...
            if self._game.combat.general_state == "units":
                self.place_target[0] = min(self._game.window.base_resolution[0] // 4, max(0, self.place_target[0]))
                self.place_target[1] = min(self._game.window.base_resolution[1], max(0, self.place_target[1]))

            if self._game.input.states["select"]:
                # hand will contain the hand for whichever deck is in use
                if self._game.combat.state == CombatState.UNIT_SELECT_TARGET:
                    unit = self._game.memory.player_troupe._units[self._game.combat.placeable_units[self.selected_col]]
                    self._game.combat.units_are_available[self.selected_col] = False
                    unit.pos = self.place_target.copy()
                    self._game.combat.leadership_points_spent += unit.tier
            ...
            if self._game.input.states["cancel"] or self._game.input.states["select"]:
                ...
                if self._game.combat.state == CombatState.UNIT_SELECT_TARGET:
                    self._game.combat.state = CombatState.UNIT_CHOOSE_CARD
                else:
                    self._game.combat.state = CombatState.ACTION_CHOOSE_CARD

        elif self._game.combat.state == CombatState.WATCH:
            if self._game.input.states["cancel"]:
                self._game.combat.state = CombatState.ACTION_CHOOSE_CARD
                self._game.combat.start_action_phase()
    ...
    def button_count(self):
        if self._game.combat.general_state == "units":
            return len(self._game.combat.placeable_units)
        elif self._game.combat.general_state == "actions":
            return len(self._game.memory.player_actions)

    def render_buttons(self, surface: pygame.Surface):
        background_surf = pygame.transform.scale(self._game.assets.ui["rounded_box"].copy(), (40, 40))
        background_surf.set_alpha(180)
        selected_surf = pygame.transform.scale(self._game.assets.ui["rounded_box_outline"], (40, 40))
        red_selected_surf = pygame.transform.scale(self._game.assets.ui["rounded_box_outline_red"], (40, 40))
        margin = 4

        if self._game.combat.general_state == "units":
            id_list = self._game.combat.placeable_units
        elif self._game.combat.general_state == "actions":
            id_list = self._game.memory.player_actions
        else:
            return None
            ...
            if self.button_scroll <= i < self.button_scroll + self.shown_buttons:
                ...
                if self._game.combat.general_state == "units":
                    obj_img = pygame.transform.scale(
                        self._game.assets.unit_animations[self._game.memory.player_troupe._units[obj_id].type]["icon"][
                            0
                        ].copy(),
                        (32, 32),
                    )
                    if not self._game.combat.units_are_available[i]:
                        obj_img.set_alpha(100)
                        can_use = False
                elif self._game.combat.general_state == "actions":
                    obj_img = pygame.transform.scale(self._game.assets.actions[obj_id].copy(), (32, 32))
                    if self._game.combat.skill_cooldowns[i]:
                        obj_img.set_alpha(100)
                        can_use = False
                ...
                # cooldown overlay
                if (not can_use) and (self._game.combat.general_state == "actions"):
                    cooldown_img = background_surf.copy()
                    cooldown_img.set_alpha(200)
                    cooldown = (
                        self._game.combat.skill_cooldowns[i]
                        / self._game.data.skills[self._game.memory.player_actions[i]]["cooldown"]
                    )
    ...
    def render(self, surface: pygame.Surface):
        ...
        if self._game.combat.state in [CombatState.UNIT_SELECT_TARGET, CombatState.ACTION_SELECT_TARGET_FREE]:
            placement_img = None
            if self._game.combat.state == CombatState.UNIT_SELECT_TARGET:
                obj_id = self._game.combat.placeable_units[self.selected_col]
                placement_img = self._game.assets.unit_animations[self._game.memory.player_troupe._units[obj_id].type][
                    "idle"
                ][0]
            if self._game.combat.state == CombatState.ACTION_SELECT_TARGET_FREE:
                # needs some assets to indicate placement of the action
                pass

            if placement_img:
                render_base = self._game.combat.camera.render_offset(self.place_target)
                surface.blit(
                    placement_img,
    ...
    def rebuild_ui(self):
        ...
        elif self._game.combat.state == CombatState.UNIT_CHOOSE_CARD:
            status = "select a unit or press X to end unit placement"
        elif self._game.combat.state == CombatState.ACTION_CHOOSE_CARD:
