...
class Ring:
    def __init__(self, game: Game,  centre: Tuple[int, int], radius: float, ):
        self.game: Game = game
        self.radius: float = radius
        self.centre: Tuple[int, int] = centre

        self.nodes: List[Node2] = []
    ...
    def generate_nodes(self, num_nodes: int):
        node_width = self._get_node_icon(NodeType.COMBAT).get_width()
        angle_between_nodes = 360 / num_nodes

        for node_count in range(num_nodes):
            vec = pygame.math.Vector2(0, -self.radius).rotate(angle_between_nodes * node_count)
